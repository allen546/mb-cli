...
def client(tmp_path: Path):
    ...
    c = ManageBacClient("bj80", domain="managebac.cn", cache=cache, retry=0)
    c.request_delay = 0.0
    ...
    def test_downgrade_to_http_is_blocked(self, client):
        ...
        with rm.Mocker() as m:
            m.get(
                f"{BASE}/student/dashboard",
                status_code=302,
                headers={"Location": "http://bj80.managebac.cn/dashboard"},
            )
            m.get("http://bj80.managebac.cn/dashboard", text="downgraded")
