...
DOWNLOAD_HOST = "https://bj80.managebac.cn"
...
def test_download_writes_attachments_and_submissions(tmp_path):
    attachments = [
        {"name": "res.pdf", "url": "https://bj80.managebac.cn/res.pdf", "source": "description"},
        {"name": "essay.pdf", "url": "https://bj80.managebac.cn/essay.pdf", "source": "submission"},
    ]
    args = _Args(tmp_path)
...
def test_download_falls_back_to_server_search_with_pages(tmp_path):
    ...
    client = _client(
        [{"name": "a.pdf", "url": "https://bj80.managebac.cn/a.pdf", "source": "description"}],
...
def test_download_partial_failure_exits_zero_and_lists_failures(tmp_path):
    attachments = [
        {"name": "good.pdf", "url": "https://bj80.managebac.cn/good.pdf", "source": "description"},
        {"name": "bad.pdf", "url": "https://bj80.managebac.cn/bad.pdf", "source": "description"},
    ]
...
def test_download_never_writes_outside_output_dir(tmp_path):
    ...
    *inside* ``--output-dir`` under the sanitized name rather than two levels
    ...
    attachments = [
        {
            "name": "../../escape.pdf",
            "url": "https://bj80.managebac.cn/escape.pdf",
            "source": "description",
        }
    ]
    client = _client(attachments)
...
def test_download_name_collisions_are_disambiguated(tmp_path):
    attachments = [
        {"name": "a.pdf", "url": "https://bj80.managebac.cn/1.pdf", "source": "description"},
        {"name": "a.pdf", "url": "https://bj80.managebac.cn/2.pdf", "source": "submission"},
    ]
    client = _client(attachments)
    args = _Args(tmp_path)
    rc, captured = _run(tmp_path, args, client)
...
def test_download_skips_entries_without_name_or_url(tmp_path):
    attachments = [
        {"name": None, "url": "https://bj80.managebac.cn/x.pdf", "source": "description"},
        {"name": "y.pdf", "url": None, "source": "description"},
        {"name": "z.pdf", "url": "https://bj80.managebac.cn/z.pdf", "source": "description"},
    ]
    client = _client(attachments)
    args = _Args(tmp_path)
    rc, captured = _run(tmp_path, args, client)
