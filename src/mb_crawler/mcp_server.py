...
def _build_client(
    ...
    if not school:
        raise RuntimeError("Missing school — pass school= or configure a profile")
    cache = ResponseCache(enabled=not refresh)
    ...
    if cookie:
        ...
        if not email_val:
            raise RuntimeError("Missing email — pass email= or configure a profile")
        if not password:
            raise RuntimeError(
                "Missing password — pass password= (interactive prompt not supported in MCP)"
            )
...
def list_tasks(
    ...
    """
    client = _build_client(school=school, domain=domain, cookie=cookie, profile=profile)
...
def submit_file(
    ...
    Args:
        task_id: Task ID or full URL (e.g. "1000026" or "https://bj80.managebac.cn/student/classes/1000023/core_tasks/1000026")
    ...
    m = _re.search(r"/student/classes/(\d+)/core_tasks/(\d+)", task_id)
    if m:
...
def list_classes(
    ...
    for task in result["upcoming"] + result["past"] + result["overdue"]:
        link = task.get("link", "")
        m = __import__("re").search(r"/student/classes/(\d+)/", link)
